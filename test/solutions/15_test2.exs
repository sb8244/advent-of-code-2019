defmodule FifteenTest do
  ...
  test "pt1" do
    ...
    Process.put(:test_input, fn ->
      ...
      Process.put(:direction, input)
      IO.inspect {"input", input, Process.get(:curr), Process.get(:map)}, limit: :infinity
      render()
      input
    end)
    ...
    Process.get(:curr_output) |> IO.inspect()
  end

  def process_output() do
    curr = Process.get(:curr)
    map = Process.get(:map)

    case Process.get(:curr_output) do
      nil ->
        "1"

      [result] ->
        Process.delete(:curr_output)
        last_direction = Process.get(:direction) |> String.to_integer()
        ...
        case result do
          ...
          "0" ->
            failed_spot = next_spot(last_direction, curr)
            map = Map.put(map, failed_spot, output_to_char(result))
            Process.put(:map, map)
            get_next_direction(map, last_direction)

          "1" ->
            successful_spot = next_spot(last_direction, curr)
            map = Map.put(map, successful_spot, output_to_char(result))
            Process.put(:map, map)
            Process.put(:curr, successful_spot)

            to_string(last_direction)
        end
    end
  end

  def get_next_direction(map, last_direction) do
    {x, y} = Process.get(:curr)

    cond do
      Map.get(map, {x - 1, y}) == nil -> "3"
      Map.get(map, {x + 1, y}) == nil -> "4"
      Map.get(map, {x, y - 1}) == nil -> "1"
      Map.get(map, {x, y + 1}) == nil -> "2"
      true ->
        next_move = rem(last_direction + 1, 4) |> map_next()
        coord = next_spot(next_move, {x, y})

        if Map.get(map, coord) != "#" do
          to_string(next_move)
        else
          get_next_direction(map, next_move)
        end
    end
  end

  def output_to_char("0"), do: "#"
  def output_to_char("1"), do: "."
  def output_to_char("2"), do: "V"

  def next_spot(1, {x, y}), do: {x, y - 1}
  def next_spot(2, {x, y}), do: {x, y + 1}
  def next_spot(3, {x, y}), do: {x - 1, y}
  def next_spot(4, {x, y}), do: {x + 1, y}
  ...
  def render() do
    ...
    min_x = Map.keys(map) |> Enum.map(& elem(&1, 0)) |> Enum.min()
    max_x = Map.keys(map) |> Enum.map(& elem(&1, 0)) |> Enum.max()
    min_y = Map.keys(map) |> Enum.map(& elem(&1, 1)) |> Enum.min()
    max_y = Map.keys(map) |> Enum.map(& elem(&1, 1)) |> Enum.max()

    for r <- (min_y..max_y), c <- (min_x..max_x) do
      IO.write(Map.get(map, {c, r}, " "))

      if c == max_x, do: IO.write("\n")
    end
  end
end
